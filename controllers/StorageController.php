...
class StorageController extends BaseController
{
    public function actionIndex()
    {
        ...
        $info = Storage::getAll($name,$pageSize);
    ...
    public function actionAdd()
    {
        if($this->request->isPost){
            $customer = $this->request->post('storage');
            $model = new Storage();
            $model->setAttributes([
                'name' => $customer['name'],
    ...
    public function actionCheckname_ajax() {
        if ($this->request->isAjax) {
            ...
            if (!empty($id)) {
                $userInfo = Storage::find()->where('name=:name and id !=:id', [':name' => $username, ':id' => $id])->one();
            } else {
                $userInfo = Storage::find()->where(['name' => $username])->one();
            }
            if (!empty($userInfo)) {
