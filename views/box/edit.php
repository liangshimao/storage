...
<div class="pad_10">
    <div class="common-form">
        <form name="myform" action="<?php echo Url::toRoute(['/box/edit','id'=>$model->id]); ?>" method="post" id="myform">
            <table width="100%" class="table_form contentWrap">
                <tr>
                    <th width="100">包装名称：</th>
                    <td><input type="text" name="box[name]" value="<?=$model->name?>" class="form-control-table width-160" id="User-name" style="display: inline" ></td>
                </tr>
                <tr>
                    <th width="100">单位：</th>
                    <td><input type="text" name="box[unit]" value="<?=$model->unit?>" class="width-160 form-control" id="User-alias" style="display: inline"></td>
                </tr>

                <tr>
                    <th width="100">价格：</th>
                    <td><input type="text" name="box[price]" maxlength="11" value="<?=$model->price?>" class="width-160 form-control" id="User-mobile" style="display: inline"></td>
                </tr>
                <tr>
                    <th width="100">排序：</th>
                ...
                <tr>
                    <th width="100">状态：</th>
                    <td>
                        <input type="radio" name="box[status]" value="0" <?=$model->status==0?'checked':'';?>>正常
                        <input type="radio" name="box[status]" value="1" <?=$model->status==1?'checked':'';?>>禁止
                    </td>
                </tr>
            </table>
