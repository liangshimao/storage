...
<div class="search-nav">
    <form class="form-inline" action="<?= Url::toRoute('/pear/index') ?>" method="get">
        <div class="form-group input-group-sm">
            <label for="txtName">梨名：</label>
            <input class="form-control ipt" id="txtName" placeholder="梨品名称" name="name" value="<?= empty($name) ? '' : $name ?>">
        </div>
        ...
        <tr>
            ...
            <td>
                <a class="btn btn-warning buttonbtn btn-info button"
                   href="javascript:window.parent.edit(1,'修改梨品信息','<?php echo Url::toRoute(['/pear/edit', 'id' => $val->id]); ?>', 600, 300)"><i
                        class="glyphicon glyphicon-edit"></i> 修改</a>

                <a class="btn btn-danger button"
                   href="javascript:confirmurl('<?= Url::toRoute(['/pear/delete', 'id' => $val->id]); ?>', '确定要刪除<?=$val->name?>吗?')"><i
...
<script type="text/javascript">
    ...
    function add()
    {
        omnipotent('edit','<?=Url::toRoute('/pear/add')?>', '添加梨品', 600, 350, 0);
    }
